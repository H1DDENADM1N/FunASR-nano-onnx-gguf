...
def load_onnx_models():
    ...
    encoder_sess = onnxruntime.InferenceSession(
        ENCODER_ONNX_PATH, sess_options=session_opts, providers=["CPUExecutionProvider"]
    )

    ctc_sess = onnxruntime.InferenceSession(
        CTC_ONNX_PATH, sess_options=session_opts, providers=["CPUExecutionProvider"]
    )
